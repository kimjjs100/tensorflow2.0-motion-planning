...
class Environment(object):
    ...
    def reset(self):
        ...
        while self.collision_init_check(self.location):
            self.location = (self.joint_range - 2.0 * self.margin) * np.random.rand(self.dim) + self.joint_min + self.margin
        ...
        while self.collision_init_check(self.goal) | self.goal_check(self.location):
            self.goal = (self.joint_range - 2.0 * self.margin) * np.random.rand(self.dim) + self.joint_min + self.margin
    ...
    def step(self, action):
        ...
        if self.collision_path_check(self.location, next_location) | self.range_check(next_location):
            self.reward = -1.0
            self.done = False
        else:
            self.location = next_location
            if self.goal_check(self.location):
                self.reward = 0.0
                self.done = True
                self.success = True
            else:
    ...
    def build_obstacle(self):
        ...
        obstacle.append({'c': np.array([0., 0., -2.]).reshape(3, 1), 'e': np.array([400., 400., 1.]).reshape(3, 1),
                         'u': self.ZYZ_Rmaxtrix(0., 0., 0.)})
    ...
    def collision_init_check(self, check_point):
        ...
        x.append(check_point + np.array([0.0, 0.0, 0.0]))
        x.append(check_point + np.array([self.margin, 0.0, 0.0]))
        x.append(check_point + np.array([-self.margin, 0.0, 0.0]))
        x.append(check_point + np.array([0.0, self.margin, 0.0]))
        x.append(check_point + np.array([0.0, -self.margin, 0.0]))
        x.append(check_point + np.array([0.0, 0.0, self.margin]))
        x.append(check_point + np.array([0.0, 0.0, -self.margin]))
    ...
    # 한 지점에 대해 충돌이 발생하는지를 체크하는 함수
    def collision_check(self, check_point):

        flag = False

        kine = self.kine_rmx52(check_point)

        _, obb, check_matrix = self.obb_rmx52(kine)

        self_collision, _, _ = self.self_collision_check(obb, check_matrix)

        env_collision, _, _ = self.env_collision_check(self.obstacle, obb)

        if self_collision | env_collision:
            flag = True

        return flag

    # 1. forward kinematics를 통해 로봇의 각 obb의 world 좌표계 기준 값을 구함
    def kine_rmx52(self, point):
